...
from annie.scoring import catalog_episode_rank, catalog_episode_score, pick_best, rank_entry
...
class CatalogEpisodeScoreTests(unittest.TestCase):
    def test_batch_beats_directors_cut(self) -> None:
        ...
        batch_rank = rank_entry(batch, WatchTarget(query="re zero"), match_queries=["re zero"])
        ...
        dc_rank = rank_entry(dc, WatchTarget(query="re zero"), match_queries=["re zero"])
        self.assertIsNotNone(dc_rank)
        dc_item = ResultItem(entry=dc, parsed=dc_rank[1], score=dc_rank[0])

        self.assertGreater(catalog_episode_rank(batch_item), catalog_episode_rank(dc_item))
        self.assertGreater(catalog_episode_score(batch_item), catalog_episode_score(dc_item))
