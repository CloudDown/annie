...
def _mal_tv_from_query(
    ...
    if chosen is None:
        return None
    # AniList : franchise via metadata ; MAL id positif pour Jikan top-anime.
    if chosen.anilist_id is not None:
        ...
        return title, tv, chosen.type, search_query
...
def _gather_with_retry(
    ...
    for attempt in range(attempts):
        try:
            catalog, options = gather_catalog(
                query, config, confirm_anime=False
            )
            if catalog:
...
def validate_target(target: ValidateTarget, config: AnnieConfig) -> dict:
    ...
    try:
        # Toujours la query utilisateur : search_query est un hint Nyaa (souvent
        # un synonyme trop court) qui re-déclencherait un mauvais pick AniList.
        catalog, _ = _gather_with_retry(target.query, config)
    except Exception as exc:
